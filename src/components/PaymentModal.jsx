...
export default function PaymentModal({
  ...
  gateways,
  onClose,
  onPay,      // onPay(gatewayName, paymentOption) — parent handles SDK initialization
  loading,
  transactions = [],
}) {
  ...
  const originalAmount = invoice ? parseFloat(invoice.amount) : 0;
  const balanceDue = invoice ? parseFloat(invoice.balance_due > 0 ? invoice.balance_due : invoice.amount) : 0;
  const currencySymbol = invoice?.currency || '$';

  // Successful transactions list of options
  const paidOptions = (transactions || [])
    .filter(tx => Number(tx.invoice_id) === Number(invoice?.id))
    .map(tx => tx.payment_option);
  ...
  const paymentPlans = [
    {
      id: '100%',
      title: 'Full Payment',
      desc: 'Pay 100% of the remaining balance due.',
      amount: balanceDue,
      isPaid: false,
      isDisabled: false,
    },
    {
      id: '50%',
      title: 'Part Payment (50%)',
      desc: '50% of the original invoice value.',
      amount: originalAmount * 0.50,
      isPaid: paidOptions.includes('50%'),
      get isDisabled() {
        return this.isPaid || this.amount > (balanceDue + 0.01);
      }
    },
    {
      id: '30%',
      title: 'Installment (30%)',
      desc: '30% of the original invoice value.',
      amount: originalAmount * 0.30,
      isPaid: paidOptions.includes('30%'),
      get isDisabled() {
        return this.isPaid || this.amount > (balanceDue + 0.01);
      }
    }
  ...
  const selectedPlan = paymentPlans.find(plan => plan.id === paymentOption) || paymentPlans[0];
  const amountDue = selectedPlan.amount;
  const formattedAmount = amountDue.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 });

  const handlePay = () => {
    if (!canProceed || loading) return;
    onPay(selected, paymentOption);
  };
        ...
        >
          ...
          <div style={{ padding: '24px 32px 28px', overflowY: 'auto', flexGrow: 1 }}>
            ...
            <div style={{
              ...
              {paymentPlans.filter(plan => !plan.isPaid).map((plan) => {
                const isSelected = paymentOption === plan.id;
                const isPlanDisabled = plan.isDisabled;
                return (
                  <button
                    key={plan.id}
                    type="button"
                    disabled={isPlanDisabled || loading}
                    onClick={() => setPaymentOption(plan.id)}
                    style={{
                      all: 'unset',
                  ...
                  >
                    <div>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                        <span style={{ fontSize: '0.8rem', fontWeight: 800, color: isSelected ? 'var(--primary)' : 'var(--text-primary)' }}>
                          {plan.title}
                        </span>
                        {plan.isPaid && (
                          <span style={{ fontSize: '0.62rem', fontWeight: 700, padding: '2px 6px', borderRadius: '4px', backgroundColor: 'rgba(16,185,129,0.12)', color: 'var(--success)' }}>
                            Paid
                          </span>
                        )}
                        {!plan.isPaid && isPlanDisabled && (
                          <span style={{ fontSize: '0.62rem', fontWeight: 700, padding: '2px 6px', borderRadius: '4px', backgroundColor: 'rgba(239,68,68,0.1)', color: 'var(--error)' }}>
                            N/A
                          </span>
                        )}
                      </div>
                      <p style={{ fontSize: '0.68rem', color: 'var(--text-muted)', margin: '0 0 12px', lineHeight: 1.35 }}>
                        {plan.desc}
                      </p>
                    </div>
                    <div>
                      <span style={{ fontSize: '1.15rem', fontWeight: 900, color: isSelected ? 'var(--accent)' : 'var(--text-primary)' }}>
                        {currencySymbol}{plan.amount.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                      </span>
                    </div>
                  </button>
                );
            ...
            {/* ── Pay Button ── */}
            {gateways.length > 0 && (() => {
              const buttonBg = canProceed && selectedMeta
                ? selectedMeta.gradient
                : 'var(--bg-tertiary)';

              const buttonShadow = canProceed && selectedMeta
